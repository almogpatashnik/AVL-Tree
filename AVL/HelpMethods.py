...
def rebalanceLoop(T, node):
    ...
    while node != None:
        ...
        if abs(bf) < 2:
            ...
            # if BF = 2 - rotate and update locally
            x = node
            node = node.parent
...
@type k: int
@param val: search for k'th item in node's subtree
@rtype: AVLNode
...
"""perform first step in double rotation
@type node: AVLNode
...
"""insert a new node with value val at position i as in BST
@type T: AVLTreeList
...
def insertNode(T, i, val):
    ...
    if T.length() == 0:
        ...
        if i == T.length():
            ...
            # insert the new node as the predecessor of the current item at position i
